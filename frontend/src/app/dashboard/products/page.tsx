...
import { Product, PaginatedResponse, ProductVariant } from '@/types';
...
export default function ProductsPage() {
  ...
  const openDrawer = async (product: Product) => {
    ...
    try {
      // Fetch full details including variants and attributes
      const res = await api.get(`/products/${product.id}`);
      setDrawerProduct(res.data);
    } catch (err) {
    ...
    <div className="space-y-5 relative">
      ...
      <div className="glass-card p-4 flex flex-wrap gap-3 items-center">
        ...
        <div className="relative">
          ...
          {showColumnDropdown && (
            <div className="absolute right-0 mt-2 w-48 rounded-xl border border-white/[0.06] shadow-2xl p-3 z-35 space-y-2" style={{ background: '#0c0c14' }}>
              <p className="text-[10px] uppercase font-bold text-white/30 tracking-wider mb-2">Ẩn/Hiện Cột</p>
              {Object.entries(visibleColumns).map(([col, val]) => (
                <label key={col} className="flex items-center gap-2 text-xs text-white/75 cursor-pointer hover:text-white">
                  <input 
                    type="checkbox" 
                    checked={val} 
      ...
      {activeDrawerId && (
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative bg-[#0c0c14] border-l border-white/[0.06] w-full max-w-lg h-full shadow-2xl flex flex-col z-50 animate-fade-in">
            ...
            {/* Tab Navigation */}
            <div className="flex px-6 border-b border-white/[0.04] bg-white/[0.01]">
              {[
