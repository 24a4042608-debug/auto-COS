...
import { Plus, Trash2, Truck, Save, X, Loader2 } from 'lucide-react';
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setShowForm(true)} className="btn-primary"><Plus size={14} /> Thêm NCC</button>
      </div>

      {showForm && (
        <form onSubmit={handleSave} className="glass-card p-5 space-y-4 max-w-2xl">
          <div className="flex items-center justify-between">
            <h3 className="font-semibold text-white">Thêm nhà cung cấp</h3>
            ...
            <tbody>
              {suppliers.map(s => (
                <tr key={s.id}>
                  ...
                  <td style={{ color: 'rgba(255,255,255,0.6)' }}>{(s as any).contact_person || '—'}</td>
                  <td className="text-xs" style={{ color: 'rgba(255,255,255,0.5)' }}>
                    {s.email || '—'} {s.phone ? `/ ${s.phone}` : ''}
                  </td>
                  <td>
                    <button onClick={() => handleDelete(s.id)} style={{ color: '#f87171', background: 'none', border: 'none', cursor: 'pointer' }}><Trash2 size={14} /></button>
                  </td>
                </tr>
              ))}
            </tbody>
