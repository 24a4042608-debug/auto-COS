  ...
  return (
    <div className="bg-[#0A0A0F] min-h-screen pb-20">
      ...
      <div className="relative w-full h-[70vh] min-h-[480px] max-h-[700px] overflow-hidden">
        ...
        {/* Back button */}
        <Link href="/film" className="absolute top-24 left-4 md:left-12 flex items-center gap-1.5 text-zinc-400 hover:text-white text-sm transition-colors group">
        ...
        <div className="absolute bottom-0 left-0 right-0 px-4 md:px-12 pb-10 max-w-screen-2xl mx-auto">
          ...
          >
            ...
            <div className="flex items-center gap-3 mb-3 flex-wrap">
              ...
              <span className="border border-zinc-600 text-zinc-400 text-xs px-2 py-0.5 rounded-sm font-mono">
                {movie.maturityRating}
              </span>
              {movie.isTrending && (
                <span className="text-[#E50914] text-xs font-bold tracking-wider uppercase border border-[#E50914]/40 px-2 py-0.5 rounded-sm">
                  Trending
                </span>
              )}
            </div>

            <h1 className="text-white font-black text-4xl md:text-6xl tracking-tight leading-tight mb-2">
              {movie.title}
            </h1>
            {movie.originalTitle && (
              <p className="text-zinc-500 text-sm italic mb-3">{movie.originalTitle}</p>
            )}
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <button
                onClick={handleWatch}
                className="flex items-center gap-3 bg-white hover:bg-zinc-200 text-black px-8 py-3.5 font-bold text-sm rounded-sm transition-all cursor-pointer hover:shadow-xl"
              >
              ...
              <button
                ...
                className={cn(
                  ...
                  bookmarked ? "bg-white border-white text-black" : "border-zinc-500 text-white hover:border-white"
                )}
              ...
              <button
                ...
                className={cn(
                  "w-12 h-12 rounded-full border-2 flex items-center justify-center transition-all cursor-pointer",
                  liked ? "bg-[#E50914]/20 border-[#E50914] text-[#E50914]" : "border-zinc-500 text-white hover:border-white"
                )}
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {movie.genres.map((g) => (
                <Link key={g} href={`/film/browse?genre=${g}`}>
                  <span className="bg-zinc-900 hover:bg-zinc-800 border border-white/10 text-zinc-300 text-xs px-3 py-1.5 rounded-sm cursor-pointer transition-colors">
                    {g}
                  </span>
                </Link>
              ))}
              ...
              <div className="flex items-center gap-3 flex-wrap">
                {movie.cast.map((actor) => (
                  <div key={actor} className="flex items-center gap-2 bg-zinc-900/60 border border-white/10 rounded-sm px-3 py-2">
                    <div className="w-7 h-7 bg-zinc-800 rounded-full flex items-center justify-center text-zinc-400">
            ...
            <div className="space-y-2">
              ...
              <button
                onClick={handleWatch}
                className="text-[#E50914] text-xs hover:text-red-300 cursor-pointer"
              >
                Continue watching →
              </button>
            </div>
      ...
      <AnimatePresence>
        {playerOpen && (
          ...
          >
            {/* Close */}
            <button
              onClick={() => { setPlayerOpen(false); setPlaying(false); }}
              className="absolute top-5 right-5 w-10 h-10 bg-zinc-900/80 hover:bg-zinc-800 border border-white/10 rounded-full flex items-center justify-center text-white cursor-pointer z-10 transition-colors"
            >
              <X className="w-5 h-5" />
            </button>

            {/* Film backdrop as video stand-in */}
            <div className="relative w-full h-full flex items-center justify-center">
              ...
              <div className="relative z-10 flex flex-col items-center gap-4">
                <motion.button
                  ...
                  className="w-20 h-20 bg-white/10 hover:bg-white/20 border border-white/20 backdrop-blur-sm rounded-full flex items-center justify-center cursor-pointer transition-all"
                >
                  {playing
              ...
              <div className="absolute bottom-0 left-0 right-0 px-6 pb-6 pt-16 bg-gradient-to-t from-black to-transparent">
                <div className="max-w-4xl mx-auto space-y-3">
                  ...
                  <div className="space-y-1">
                    <div
                      className="w-full h-1.5 bg-zinc-700/80 rounded-full cursor-pointer group"
                      onClick={(e) => {
                        const rect = e.currentTarget.getBoundingClientRect();
                        const newPct = Math.round(((e.clientX - rect.left) / rect.width) * 100);
                        setProgress(newPct);
                        addToHistory(movie.id, newPct);
                      }}
                    >
                      <div
                        className="h-full bg-[#E50914] rounded-full relative transition-all duration-100"
                        style={{ width: `${progress}%` }}
                      >
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-5">
                      <button className="text-white/70 hover:text-white cursor-pointer transition-colors">
